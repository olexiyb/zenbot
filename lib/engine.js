...
module.exports = function (s, conf) {
  ...
  function updatePeriod(trade) {
    ...
    if (trade.side === 'buy') {
      s.period.buy_volume += trade.size
    } else if (trade.side === 'sell') {
      s.period.sell_volume += trade.size
    }
    s.period.delta_volume = s.period.buy_volume - s.period.sell_volume
  ...
  function executeOrder (order, trade_type) {
    ...
    eventBus.emit('tradeExecuted', my_trade)
  }
  ...
  function onTrade(trade, is_preroll, cb) {
    ...
    if(trade.time > s.period.close_time ||
      ...
      s.strategy.onPeriod.call(s.ctx, s, function () {
        ...
        //s.action = null
        eventBus.emit('period_added', s.period)
        s.signal = null
        if (trade.time > s.period.close_time) {
